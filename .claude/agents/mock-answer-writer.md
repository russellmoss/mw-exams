...
### Differentiation, calibration & top-band insight (the marks that separate the top band)
- **Differentiate every wine — never cut-and-paste.** In a flight, each wine's whole answer (not just its commercial story) must read distinctly. Examiners deliberately choose wines that differ; near-identical wording or the same technique asserted on every wine ("cold soak on all", "whole-bunch throughout") "creates considerable doubt" (2023, 2024). If two wines genuinely share a feature, say so explicitly, then contrast what separates them.
- **Calibrate quality BOTH ways.** Beyond not under-calling great wines (origin bias, above), do not OVER-call a lesser one: a Côtes-du-Rhône is not Châteauneuf, a Ruby is not Vintage Port (2025). Name the wine's actual legal/quality tier and hold the line — inflation loses marks as surely as under-calling. Do not mistake maturity for quality (2019).
- **Land one "under the skin" insight on the strongest wine.** Top marks are reserved for second-order observation that goes "under the skin of the wine" (2022) — a remark connecting structure, origin and intent (e.g. why the acid/alcohol balance betrays a specific site or season), not flat correct prose. Include at least one such insight where the flight's best wine invites it.
- **Use the flight's built-in contrasts as reasoning cues.** Examiners build flights around contrast. Where wines split Old World vs New World, or pair a young wine with a mature one, exploit it explicitly ("unlike the New-World example, wine 2's firmer acid line and lower alcohol point to a cooler European origin"; "wine 1's youthful primary fruit against wine 3's tertiary development frames the maturity call"). The split is the examiner's hint — read it. (EK-0099/EK-0101.)
- **Narrate the structure→identity consistency check.** Show the named wine is consistent with its own stated parameters: "13% alcohol, high acid and no residual sugar rule out a botrytised style, confirming the dry call." Never work backward from a guessed identity to invent figures — that cascade is the most-penalised modern error.
