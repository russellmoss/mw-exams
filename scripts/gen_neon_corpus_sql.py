...
stmts.append("""CREATE TABLE corpus.wines (
  wine_id text PRIMARY KEY, year int, paper int, slot int, is_recent_window boolean,
...
cols = "(wine_id,year,paper,slot,is_recent_window,variety,country,world,region,vintage,age_at_exam,style_category,curveball_level,price_band,benchmark_status,commercial_tier,data)"
for batch in chunked(W, 120):
    vals = []
    for w in batch:
        vals.append("(" + ",".join([
            s(w["wine_id"]), s(w["year"]), s(w["paper"]), s(w["slot"]), s(w["is_recent_window"]),
...
stmts.append("""CREATE TABLE corpus.questions (
  qid text PRIMARY KEY, year int, paper int, n int, is_recent_window boolean,
...
cols = "(qid,year,paper,n,is_recent_window,flight_size,total_marks,expected_marks,marks_ok,n_subquestions,family,subcategory,data)"
for batch in chunked(Q, 120):
    vals = []
    for q in batch:
        vals.append("(" + ",".join([
            s(q["qid"]), s(q["year"]), s(q["paper"]), s(q["n"]), s(q["is_recent_window"]),
...
stmts.append("""CREATE TABLE corpus.subquestions (
  subq_id text PRIMARY KEY, qid text, year int, paper int, n int, is_recent_window boolean,
...
cols = "(subq_id,qid,year,paper,n,is_recent_window,flight_size,label,type,marks_each,marks_count,marks_sum,marks_pct_of_question,data)"
for batch in chunked(S, 120):
    vals = []
    for x in batch:
        vals.append("(" + ",".join([
            s(x["subq_id"]), s(x["qid"]), s(x["year"]), s(x["paper"]), s(x["n"]), s(x["is_recent_window"]),
