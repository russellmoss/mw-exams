...
export function AudioVisualizer({ getLevel, state, size = 220, animate = true }: Props) {
  ...
  React.useEffect(() => {
    ...
    // Cellar tokens (DESIGN.md). There is no --accent-soft in this palette, so the inner stop of
    // each glow ring falls back to the accent at zero alpha — which is what --accent-soft was doing
    // visually anyway. --muted, not --text-muted, is this app's spelling.
    const accent = readToken("--accent", "#d97706");
    const accentSoft = readToken("--accent-hover", accent);
    const muted = readToken("--muted", "#78716c");
