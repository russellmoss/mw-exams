// Structure-aware markdown chunking. PORTED VERBATIM from the Wine-inventory knowledge stack
// (plan 079 / plan 100), which is why the comments below reference plans and councils that do not
// exist in this repo — they are kept deliberately, because they record WHY the code is shaped this
// way and re-deriving that would be pure loss.
//
// Why port it rather than write one: the fortified corpus is built from Tavily `extract` output,
// which is markdown, and this chunker is markdown-heading aware. It splits on heading hierarchy,
// never splits a markdown table, prepends the section breadcrumb into every chunk (so the chunk
// carries its own context into the embedding), and carries a sentence overlap between neighbours.
// A hand-rolled splitter would have been worse in all four respects.
//
// The one thing that does NOT apply here: CHUNKER_VERSION exists in the source system to invalidate
// an incremental-crawl content hash. This corpus has no incremental crawl — it is rebuilt by hand —
// so the constant is retained only so the ported code stays byte-comparable with its origin.
