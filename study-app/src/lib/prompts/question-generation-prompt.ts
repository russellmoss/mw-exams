...
SINGLE-AXIS COHERENCE (HARD — validator rejects with NO_COMMON_AXIS): a Paper 3 flight must hang on ONE testable axis, so the candidate knows what is being tested. Every wine maps to a top-level category — {sparkling, rosé, fortified, sweet-unfortified, oxidative/flor-aged, dry-still}. A flight spanning THREE OR MORE of these categories (e.g. a fortified Muscat + a rosé + a sweet Monastrell, or a sherry + a rosé + a sparkling) is a grab-bag with no single axis and is REJECTED. You may use exactly TWO categories ONLY as a deliberate contrast whose shared axis you NAME in the stem or a sub-part (e.g. "both are sweet wines", "compare the methods of fortification"); otherwise keep all wines in one category. Note that sweet/fortified wines almost always also carry a "state the residual sugar (g/L) and/or alcohol level" ask worth a few marks each — include it when the axis is sweetness or fortification.
