...
- A LOW-RECOGNITION APPELLATION of a famous variety, when a high-recognition one exists. When Sangiovese serves as the anchor, use Chianti Classico, Brunello di Montalcino, Rosso di Montalcino or Vino Nobile di Montepulciano — NOT Morellino di Scansano, Rosso Conero or Montescudaio (reviewer rejection, attempt #510: the exam anchors on the appellations candidates actually train on). Morellino also blends beyond its Sangiovese base, so never place it under a "single grape variety/varieties" stem.
...
## RS / SWEET FLIGHT STRUCTURE (mandatory when the stem links every wine by residual sugar)
When the stem declares that every wine has residual sugar and awards marks for HOW that sugar was achieved, three structural rules apply. They correct the exact faults a reviewer binned on a six-wine all-curveball RS flight (attempt #596): five curveballs against one anchor, three botrytis wines bunched together, and no RS-in-g/L or ABV numerical sub-part.

1. BANKER FLOOR (Change A — EK-0029, EK-0097). In a flight of N wines all linked by residual sugar, at least ⌈N/3⌉ wines MUST be low-curveball ANCHORS — wines a well-prepared candidate is expected to name cold. So a 6-wine RS flight needs at least 2 anchors, a 4-wine flight at least 2, a 3-wine flight at least 1. Do NOT classify a wine as an anchor unless it is at least as commercially familiar as De Bortoli Noble One or Château Coutet. Model anchors: De Bortoli Noble One, Tokaji Aszú, Inniskillin Icewine, Château Coutet, a Sauternes Premier Cru, Rutherglen Muscat. NOT anchors (these are curveballs, however good): Bonnezeaux, Moscadello di Montalcino, Kiona/Washington late-harvest Riesling, Baden Weissburgunder Spätlese, Rosenmuskateller Beerenauslese. A six-wine flight running five curveballs against one anchor inverts the exam's real shape and is a generation error.

2. MECHANISM SPREAD (Change B — EK-0039, EK-0080). Sub-part (b) tests the mechanism by which RS was achieved, so the flight must SPREAD mechanisms. No more than TWO wines may share the same primary mechanism. Before output, explicitly count each mechanism across the flight: botrytis (noble rot); late-harvest / passerillage; icewine / cryoextraction; fermentation arrest (naturally late-picked Prädikat left with RS); VDN / fortification mutage; dried-grape passito (Vin Santo). A mechanism appearing in THREE or more wines in a six-wine flight is a generation error — e.g. Bonnezeaux + Noble One + a Beerenauslese are all botrytis, which is three, too many. The real exam spreads mechanisms precisely to create the contrast this sub-part rewards (2013 P3 Q3, 2015 P3 Q3, 2024 P3 Q4).

3. RS / ABV NUMERICAL ASK (EK-0184). When all wines are linked by residual sugar and sub-part (b) asks how the sugar was achieved, the flight MUST also carry a discrete numerical sub-part asking the candidate to "State the level of residual sugar (g/L)" and/or "State the alcohol level" — worth 2-3 marks per wine, on its own single-line bullet. Every comparable real RS flight from 2012 to 2025 includes it (2013 P3 Q3, 2015 P3 Q3, 2024 P3 Q4). Omitting it is a structural gap. This is the one place the 2-3 mark numerical bullet is expected; keep the written sub-parts at 5+ marks as usual and re-check that the per-wine total is still 25.
...
MANDATORY OLD WORLD ANCHOR — classic WHITE varieties (F1 same-variety flights spanning 2+ countries —
EK-0169, STRONG SIGNAL). Any "same single grape variety" flight of a classic WHITE variety whose wines
come from two or more countries MUST include at least one Old World anchor wine from the variety's
classic European home region. A PAIR counts: two New World Rieslings from different countries is the
same defect as four New World Chardonnays, and was binned by the reviewer as such. The rule:
...
multi-country same-variety WHITE flight with no such anchor, pairs included.
