...
const val = [
  ...
  // R11 (fa_65): residual sugar declared as a P1 premise. Neither wine carries an RS value or a style
  // tag — exactly the real key of gen_p1_F6_1779997829060, and the reason STEM_PREDICATE_MISMATCH (which
  // needs one of those to prove a wine is dry) could never catch this question. R11 reads the shape.
  // (The two flight-composition flags are the fixture's own doing — a bare "Loire Valley" key resolves
  // no banker — and are listed rather than engineered away so the sweetness verdict stays the subject.)
  { name: "P1 residual-sugar premise", ok: false, sm: "per-wine", viol: ["flight-composition:hard", "flight-composition:hard", "sweetness-out-of-paper:hard"], q: { questionId: "c11", paper: 1, family: "F6", questionText: "Wines 1 and 2 are from the same region. Both wines have residual sugar.\n\na) Identify the grape variety and the region of origin. (10 marks)\nb) Comment on the method of production, with particular reference to how the residual sugar was achieved. (2 x 20 marks)", totalMarks: 50, wines: [V(1, ["Chenin Blanc"], "Loire Valley", "France"), V(2, ["Chenin Blanc"], "Loire Valley", "France")] } },
  // Same device in Paper 3, where the corpus actually uses it: no sweetness verdict at all.
  { name: "P3 residual-sugar premise ok", ok: true, sm: "per-wine", viol: [], q: { questionId: "c12", paper: 3, family: "F6", questionText: "Wines 1 and 2 all have residual sugar. (2 x 25 marks)", totalMarks: 50, wines: [V(1, ["Semillon"], "Sauternes", "France", { style: "botrytis sweet" }), V(2, ["Furmint"], "Tokaj", "Hungary", { style: "botrytis sweet" })] } },
